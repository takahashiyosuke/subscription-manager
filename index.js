...
function getCurrentTimeInTimezone(timezone = 'UTC') {
  try {
    // Workers 環境下 Date 始終存儲 UTC 時間，這裡直接返回當前時間對象
    return new Date();
  } catch (error) {
...
// 獲取指定時區的年/月/日/時/分/秒，便於避免重復的 Intl 解析邏輯
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">使用搜索與分類快速定位訂閱，開啟農歷顯示可同步查看農歷日期</p>
      </div>
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 w-full">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:flex-1 lg:max-w-2xl">
          <div class="relative flex-1 min-w-[200px] lg:max-w-md">
            <input type="text" id="searchKeyword" placeholder="搜索名稱、類型或備注..." class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
      ...
      <form id="subscriptionForm" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label for="category" class="block text-sm font-medium text-gray-700 mb-1">分類標簽</label>
            <input type="text" id="category" placeholder="例如：個人、家庭、公司"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
            <p class="mt-1 text-xs text-gray-500">可輸入多個標簽並使用“/”分隔，便於篩選和統計</p>
            <div class="error-message text-red-500"></div>
          </div>
        </div>
          ...
          <div class="md:col-span-2">
            ...
            <div class="relative">
              <input type="text" id="startDate"
                ...
                placeholder="YYYY-MM-DD 或點擊右側圖標選擇">
...
const lunarBiz = {
  ...
  daysToLunar(lunar) {
    ...
    function renderSubscriptionTable() {
      ...
      filtered.forEach(subscription => {
        ...
        const calendarTypeHtml = subscription.useLunar
          ? '<div class="text-xs text-purple-600 mt-1">日歷類型：農歷</div>'
          : '<div class="text-xs text-gray-600 mt-1">日歷類型：公歷</div>';
        ...
        row.innerHTML =
          '<td data-label="名稱" class="px-4 py-3"><div class="td-content-wrapper">' +
            ...
            notesHtml +
          '</div></td>' +
          '<td data-label="類型" class="px-4 py-3"><div class="td-content-wrapper space-y-1">' +
	...
	// 4. 新增修改，監聽 useLunar 復選框變化時也自動重新計算
    ...
    async function showSystemTime() {
      try {
        ...
        function formatTime(dt, tz) {
          return dt.toLocaleString('zh-TW', { timeZone: tz, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' });
        }
...
<body class="bg-gray-100 min-h-screen">
      ...
      <form id="configForm" class="space-y-8">
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <select id="timezone" name="timezone" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 bg-white">
            <option value="UTC">世界標准時間（UTC+0）</option>
            <option value="Asia/Shanghai">中國標准時間（UTC+8）</option>
            ...
            <p class="mt-1 text-sm text-gray-500">選擇需要使用時區，系統會按該時區計算剩餘時間（提醒 Cron 仍基於 UTC，請在 Cloudflare 控制台換算觸發時間）</p>
          </div>
        </div>
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-md p-3 text-sm text-indigo-700">
              ...
              <p>Cloudflare Workers Cron 以 UTC 計算，例如北京時間 08:00 需設置 Cron 為 <code>0 0 * * *</code> 並在此填入 08。</p>
              <p class="mt-1">若 Cron 已設置為每小時執行，可用該字段限制實際發送提醒的小時段。</p>
            </div>
          </div>
          <div class="mb-6">
            ...
            <div class="mt-2 flex flex-wrap gap-4">
              ...
              <a href="https://webhook.site" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm">
                <i class="fas fa-external-link-alt ml-1"></i> Webhook 調試工具
              </a>
          ...
          <div class="mb-6">
            ...
            <div class="mt-1 flex flex-col sm:flex-row sm:items-center gap-3">
              <input type="text" id="thirdPartyToken" placeholder="建議使用隨機字符串，例如：iH5s9vB3..."
            ...
            <p class="mt-1 text-sm text-gray-500">調用 /api/notify/{token} 接口時需攜帶此令牌；留空表示禁用第三方 API 推送。</p>
          </div>
          ...
          <div id="webhookConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">請填寫自建服務或第三方平台提供的 Webhook 地址，例如 <code>https://your-webhook-endpoint.com/path</code></p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">支持變量: {{title}}, {{content}}, {{timestamp}}。留空使用預設格式</p>
              </div>
            </div>
            <div class="flex justify-end">
          ...
          <div id="barkConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Bark 服務器地址，預設為官方服務器，也可以使用自建服務器</p>
              </div>
              <div>
  ...
  <script>
    ...
    async function loadConfig() {
      try {
        ...
        if (notificationHoursInput) {
          // 將通知小時數組格式化為逗號分隔的字符串，便於管理員查看與編輯
    ...
    function generateTimezoneOptions(selectedTimezone = 'UTC') {
      ...
      const timezones = [
        { value: 'UTC', name: '世界標准時間', offset: '+0' },
        { value: 'Asia/Shanghai', name: '中國標准時間', offset: '+8' },
    ...
    document.getElementById('generateThirdPartyToken').addEventListener('click', () => {
      try {
        ...
        showToast('已生成新的第三方 API 令牌，請保存配置後生效', 'info');
      } catch (error) {
...
const api = {
  async handleRequest(request, env, ctx) {
    ...
    if (path === '/test-notification' && method === 'POST') {
      try {
        ...
        if (body.type === 'telegram') {
          ...
          message = success ? 'Webhook 通知發送成功' : 'Webhook 通知發送失敗，請檢查配置';
    ...
    if (path.startsWith('/notify/')) {
      ...
      // 允許通過路徑、Authorization 頭或查詢參數三種方式傳入訪問令牌
      ...
      if (!expectedToken) {
        return new Response(
          JSON.stringify({ message: '第三方 API 已禁用，請在後台配置訪問令牌後使用' }),
...
async function sendWebhookNotification(title, content, config, metadata = {}) {
  try {
    ...
    console.log('[Webhook通知] 開始發送通知到: ' + config.WEBHOOK_URL);
    ...
    const tagsLine = tagsArray.length ? '標簽：' + tagsArray.join('、') : '';
    const timestamp = formatTimeInTimezone(new Date(), config?.TIMEZONE || 'UTC', 'datetime');
    const formattedMessage = [title, content, tagsLine, `發送時間：${timestamp}`]
    ...
    if (config.WEBHOOK_TEMPLATE) {
      try {
        ...
        console.warn('[Webhook通知] 消息模板格式錯誤，使用預設格式');
        requestBody = {
          title,
          content,
          tags: tagsArray,
    ...
    console.log('[Webhook通知] 發送結果:', response.status, result);
    return response.ok;
  } catch (error) {
    console.error('[Webhook通知] 發送通知失敗:', error);
    return false;
  }
}
...
async function sendNotificationToAllChannels(title, commonContent, config, logPrefix = '[定時任務]', options = {}) {
  const metadata = options.metadata || {};
    ...
    if (config.ENABLED_NOTIFIERS.includes('webhook')) {
        ...
        console.log(`${logPrefix} 發送Webhook通知 ${success ? '成功' : '失敗'}`);
    }
...
async function checkExpiringSubscriptions(env) {
  try {
    ...
    const timezone = config?.TIMEZONE || 'UTC';
    const currentTime = getCurrentTimeInTimezone(timezone);
    console.log('[定時任務] 開始檢查即將到期的訂閱 UTC: ' + new Date().toISOString() + ', ' + timezone + ': ' + currentTime.toLocaleString('zh-TW', {timeZone: timezone}));
...
for (const subscription of subscriptions) {
  ...
  if (subscription.useLunar) {
    ...
    console.log('[定時任務] 訂閱 "' + subscription.name + '" 到期日期: ' + expiryDate.toISOString() + ', 農歷轉換後午夜時間: ' + new Date(lunarMidnight).toISOString() + ', 剩餘天數: ' + daysDiff);
    ...
    if (daysDiff < 0 && subscription.periodValue && subscription.periodUnit && subscription.autoRenew !== false) {
      ...
      do {
        ...
        console.log('[定時任務] 訂閱 "' + subscription.name + '" 更新到期日期: ' + newExpiryDate.toISOString() + ', 農歷轉換後午夜時間: ' + new Date(newLunarMidnight).toISOString() + ', 剩餘天數: ' + daysDiff);
      } while (daysDiff < 0);
    ...
    if (daysDiff < 0 && subscription.periodValue && subscription.periodUnit && subscription.autoRenew !== false) {
      ...
      while (newExpiryMidnight < currentMidnight) {
        console.log('[定時任務] 新計算的到期日期 ' + newExpiryDate.toISOString() + ' (時區轉換後午夜: ' + new Date(newExpiryMidnight).toISOString() + ') 仍然過期，繼續計算下一個周期');
        if (subscription.periodUnit === 'day') {
  ...
  if (daysDiff < 0 && subscription.autoRenew === false) {
    ...
    if (expiringSubscriptions.length > 0) {
      if (!shouldNotifyThisHour) {
        console.log('[定時任務] 當前小時 ' + currentHour + ' 未配置為推送時間，跳過發送通知');
...
<body>
  ...
  async scheduled(event, env, ctx) {
    ...
    const timezone = config?.TIMEZONE || 'UTC';
    const currentTime = getCurrentTimeInTimezone(timezone);
    console.log('[Workers] 定時任務觸發 UTC:', new Date().toISOString(), timezone + ':', currentTime.toLocaleString('zh-TW', {timeZone: timezone}));
